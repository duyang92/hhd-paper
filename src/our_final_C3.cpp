...
void init(BUCKET *b, int bucket_num)
{
    for (int i = 0; i < bucket_num; i++)
    {
        ...
        for (int j = 0; j < CELL_NUM; j++)
        {
            ...
            (b+i)->cells[j].right_right_child_count=0;
            (b+i)->cells[j].left_right_child_count=0;
        }
    }
}
uint32_t find_cell(BUCKET *b, int bucket_idx, uint32_t key,uint32_t level,int threshold)
{
    ...
    if(b[bucket_idx].empty_cell>0)
    {
        ...
        if(key&(1<<(level-1)))
        {
            ...
            if(key&(1<<(level-2)))
            {
                b[bucket_idx].cells[empty_cell_idx].right_right_child_count=1;
            }
        }
        else
        {
            if(key&(1<<(level-2)))
            {
                b[bucket_idx].cells[empty_cell_idx].left_right_child_count=1;
            }
        }
    ...
    for (int i = 0; i < CELL_NUM; i++)
    {
        if (b[bucket_idx].cells[i].level==level&&(b[bucket_idx].cells[i].key&masks[level])== masked_key)
        {
            ...
            if(key&(1<<(level-1)))
            {
                ...
                if(key&(1<<(level-2)))
                {
                    b[bucket_idx].cells[i].right_right_child_count++;
                }
            }
            else
            {
                if(key&(1<<(level-2)))
                {
                    b[bucket_idx].cells[i].left_right_child_count++;
                }
            }
            return 0; 
        }
    ...
    if(is_inserted==0&&min_val_cell_idx!=-1)
    {
        if (min_cell_val==0||(rand() < RAND_MAX/min_cell_val))
        {
            ...
            b[bucket_idx].cells[min_val_cell_idx].left_right_child_count=0;
            b[bucket_idx].cells[min_val_cell_idx].right_right_child_count=0;
        }
    }
    return 0;
}
...
void insert_bit(BUCKET *b, int bucket_num, uint32_t key,int threshold)
{
    ...
    int high_level=hash_level-hash_level%3+2;
    if(high_level>=DEPTH)
    {
        high_level=DEPTH-1;
    }
...
std::vector<std::pair<uint64_t,uint32_t>> query(BUCKET *b,int bucket_num,int threshold)
{
    ...
    for(int i=0;i<bucket_num;i++)
    {
        for(int j=0;j<CELL_NUM;j++)
        {
            ...
            level_cell_idx[level].push_back((i*CELL_NUM+j)*7);
            level_cell_idx[level-1].push_back((i*CELL_NUM+j)*7+1);
            level_cell_idx[level-1].push_back((i*CELL_NUM+j)*7+2);
            level_cell_idx[level-2].push_back((i*CELL_NUM+j)*7+3);
            level_cell_idx[level-2].push_back((i*CELL_NUM+j)*7+4);
            level_cell_idx[level-2].push_back((i*CELL_NUM+j)*7+5);
            level_cell_idx[level-2].push_back((i*CELL_NUM+j)*7+6);
        }
    }
    for(int i=0;i<DEPTH;i++)
    {
        ...
        for(j=0;j<level_cell_idx[i].size();j++)
        {
            ...
            int bucket_idx=cell_idx/7/CELL_NUM;
            int in_bucket_cell_idx=(cell_idx/7)%CELL_NUM;
            uint64_t key=(b+bucket_idx)->cells[in_bucket_cell_idx].key&masks[i];
            uint32_t size=(b+bucket_idx)->cells[in_bucket_cell_idx].count;
            if(i%3==1&&cell_idx%7==1)
            {
                key=key&(~(1<<i));
                size=(b+bucket_idx)->cells[in_bucket_cell_idx].count-(b+bucket_idx)->cells[in_bucket_cell_idx].right_child_count;
            }
            else if(i%3==1&&cell_idx%7==2)
            {
                key=key|(1<<i); 
                size=(b+bucket_idx)->cells[in_bucket_cell_idx].right_child_count;
            }
            else if(i%3==0&&cell_idx%7==3)
            {
                key=key&(~(1<<(i+1)));
                key=key&(~(1<<i));
                size=(b+bucket_idx)->cells[in_bucket_cell_idx].count-(b+bucket_idx)->cells[in_bucket_cell_idx].right_child_count-(b+bucket_idx)->cells[in_bucket_cell_idx].left_right_child_count;
            }
            else if(i%3==0&&cell_idx%7==4)
            {
                key=key&(~(1<<(i+1)));
                key=key|(1<<i); 
                size=(b+bucket_idx)->cells[in_bucket_cell_idx].left_right_child_count;
            }
            else if(i%3==0&&cell_idx%7==5)
            {
                key=key|(1<<(i+1));
                key=key&(~(1<<i));
                size=(b+bucket_idx)->cells[in_bucket_cell_idx].right_child_count-(b+bucket_idx)->cells[in_bucket_cell_idx].right_right_child_count;
            }
            else if(i%3==0&&cell_idx%7==6)
            {
                key=key|(1<<(i+1));
                key=key|(1<<i);
                size=(b+bucket_idx)->cells[in_bucket_cell_idx].right_right_child_count;
            }
            key=(key<<32)+i;
            flow_size[key]+=size;
            ...
            {
                HHH.push_back(std::make_pair(key,flow_size[key]*(DEPTH)/D/3));
                for(auto y:toerase)
                {
                    nearest.erase(y);
                }
