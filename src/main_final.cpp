...
int main(int argc,char *argv[])
{
    ...
    std::cout<<BUCKET_NUM<<" "<<threshold<<std::endl;
    BUCKET b[BUCKET_NUM];
    init(b,BUCKET_NUM);
	std::ifstream fin;
    ...
    threshold=threshold*D*2/(DEPTH+1);
	...
	for(int i=0;i<BUCKET_NUM;i++)
    {
        for(int j=0;j<CELL_NUM;j++)
        {
            ...
            if(actual_HHH.find(x)!=actual_HHH.end())
            {
				level_sum[level]++;
                actual_HHH_in_cell++;
                relative_error+=abs((double)cell_val*(DEPTH+1)/D/2-actual_HHH_size[x])/actual_HHH_size[x];
			...
			if(level<DEPTH-1)
			{
				...
				if(actual_HHH.find(left_child)!=actual_HHH.end())
				{
                    ...
                    relative_error+=abs((double)left_cnt*(DEPTH+1)/D/2-actual_HHH_size[left_child])/actual_HHH_size[left_child];
					level_sum[level-1]++;
				}
				if(actual_HHH.find(right_child)!=actual_HHH.end())
				{
                    ...
                    relative_error+=abs((double)right_cnt*(DEPTH+1)/D/2-actual_HHH_size[right_child])/actual_HHH_size[right_child];
					level_sum[level-1]++;
				}
			}
        }
