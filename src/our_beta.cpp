...
uint32_t find_cell(BUCKET *b, int bucket_idx, uint32_t key,uint32_t level,int threshold)
{
    ...
    else if (is_inserted==0&&has_empty_cell == 0)
    {
        ...
        if (min_cell_val*DEPTH/D<threshold&&rand() < RAND_MAX/min_cell_val)
        {
            uint64_t key1=(b + bucket_idx)->cells[min_val_cell_idx].key;
//                 if(query_keys.find(key1)!=query_keys.end())
//                 {
    ...
    { 
        ...
        for (int i = 0; i < CELL_NUM; i++)
        {
            if ((b + bucket_idx)->cells[i].is_empty==0&&(b + bucket_idx)->cells[i].level==hash_level&&((b + bucket_idx)->cells[i].key&masks[hash_level])== (x>>32))
            {
                is_inserted=1;
                (b + bucket_idx)->cells[i].count+=1;
//                 if(hash_level<DEPTH-1&&k==0&&(b + bucket_idx)->cells[i].count*DEPTH/D>=threshold)
//                 {
//                     heavy_child_size=(b + bucket_idx)->cells[i].count;
//                 }
//                 if(k>0)
//                 {
//                     (b + bucket_idx)->cells[i].heavy_child_size=std::max(heavy_child_size,(b + bucket_idx)->cells[i].heavy_child_size);
//                 }
                break; 
            }
            else if ((b + bucket_idx)->cells[i].is_empty)
...
//                 }
//             }
            if (min_cell_val*DEPTH/D<threshold&&rand()* min_cell_val < RAND_MAX)
            {
                
                (b + bucket_idx)->cells[min_val_cell_idx].count=min_cell_val;
                (b + bucket_idx)->cells[min_val_cell_idx].key=key;
                (b + bucket_idx)->cells[min_val_cell_idx].level=hash_level;
                (b + bucket_idx)->cells[min_val_cell_idx].flag=1;
                (b + bucket_idx)->cells[min_val_cell_idx].heavy_child_size=0;
