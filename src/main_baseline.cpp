...
//     }
	...
	for(int i=0;i<BUCKET_NUM;i++)
    {
        for(int j=0;j<CELL_NUM;j++)
        {
            ...
            if(actual_HHH.find(x)!=actual_HHH.end())
            {
				level_sum[level]++;
                actual_HHH_in_cell++;
                relative_error+=abs((double)cell_val*DEPTH-actual_HHH_size[x])/actual_HHH_size[x];
	...
	time_t t;
	std::vector<std::pair<uint64_t,uint32_t>> output=query_test(b,BUCKET_NUM,threshold);
